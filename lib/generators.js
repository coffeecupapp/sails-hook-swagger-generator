...
function _generateDefinitions(models) {
    ...
    _.forEach(models, function (model) {
        // Do tagging for  all models. Ignoring associative tables
        if (model.globalId) {
            //identity = model.identity.charAt(0).toUpperCase() + model.identity.slice(1);
            defintions[model.globalId] = parsers.attributes(model.attributes);
        }
    });
...
function _generateRoutes(controllers, config, tags) {
    ...
    controllers = requireDir(config.appPath + '/api/controllers');

    var routes = parsers.routes(config.routes);

    _.forEach(controllers, function (controller, identity) {

        // identity = identity.replace('Controller', '').toLowerCase();
        var name = identity.replace('Controller', '');
        identity = name.toLowerCase();
        var url = pluralize(name.charAt(0).toLowerCase() + name.slice(1));
        ...
        _.forEach(controller, function (value, action) {
            ...
            _.forEach(routes[name], function (value, key) {
                if (typeof routes[name][key] === 'undefined') {
                    routes[name].splice(key, 1);
                }
            });

            //here we check if it's in our custom routes if so don't add, simply move to the next one
            if (_.findIndex(routes[name], {'action': action}) < 0) {
                routes[name].push({
 ...
 * @private
 */
function _generatePaths(generated_routes, tags, parameterList, specifications, config) {
    var paths = {};

    var customBlueprintMaps = config.blueprint_parameters;
    var mapping = config.parameterMapping;
    var ignoredRoutes = config.ignoredRoutes;
    ...
    _.forEach(generated_routes, function (routes, identity) {
        _.forEach(routes, function (route) {
            ...
            if (route) {
                ...
                var method_path = route.http_method.toUpperCase() + ' ' + route.path;
                if (ignoredRoutes.indexOf(method_path) !== -1) {
                    return;
                }
                ...
                var path = {
                    summary: route.summary ? route.summary.replace("{identity}", name) : '',
                    description: route.description && typeof route.description === 'string' ? route.description.replace("{identity}", name) : '',
                    tags: tag ? [tag.name] : [name],
                ...
                if (route.action === "create" || route.action === "update") {
                    ...
                    if (!_.find(path.parameters, {name: 'body'})) {
                        path.parameters.push({
                            ...
                            "in": "body",
                            "required": true,
                            "description": "An object defining our schema for " + name,
                ...
                if (route.action === 'find') {
                    path.parameters = path.parameters.concat(_getParamsFromConfigurationMapping(name, mapping, parameterList))
                }

                if (route.action === "create" || route.action === "update" || route.action === 'find' || route.action === 'findOne') {
                    path.responses["200"].schema = {"$ref": "#/definitions/" + name};
                }
...
/**
 * this is used to create params based on config mapping
 * @param name
 * @param config
 * @param parameterList
 * @returns {Array}
 * @private
 */
function _getParamsFromConfigurationMapping(name, mapping, parameterList) {
    var list = parameterList;
    var params = [];

    _.forEach(mapping[name], function (parameter) {
