...
function _parseRoutes(routes) {
    ...
    _.forEach(routes, function (controller_action, method_path) {
        ...
        var model_identity = controller_action.substring(0, controller_action.indexOf("Controller"));
        ...
        var http_method = method_path[0].trim().toLowerCase()
        var action = controller_action.split('.')[1];
        ...
        var property = _.defaults(swagger_property, {
            ...
            summary: http_method.charAt(0).toUpperCase() + http_method.slice(1) + ' ' + model_identity.charAt(0).toUpperCase() + model_identity.slice(1) + ' ' + action,
            description: '',
        ...
        if (property && property.http_method !== 'post' && property.http_method !== 'put') {
